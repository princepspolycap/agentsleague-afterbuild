...
| Dilemma gates | "The dilemmas aren't canned - the narrator model writes a venture-specific tradeoff from the artifact just sealed; offline forks get one deterministic, venture-aware fallback that varies by stage and cites the rival's evolving pressure." | `tools/server.py` (`/api/dilemma`), `tools/dilemma_generator.py` (`build_stage_dilemma`) |
